    ...
    public static partial class PingComponentSystem
    {
        ...
        private static async ETTask PingAsync(this PingComponent self)
        {
            ...
            while (true)
            {
                try
                {
                    await fiber.Root.GetComponent<TimerComponent>().WaitAsync(self.AdjustIntervalTime);
        ...
        ///<summary>调节更新帧同步的更新频率</summary>
        public static async ETTask SendMapPing(this PingComponent self,  Session session )
        {
            //获取 客户端当前的时间戳
            long time1 = TimeInfo.Instance.ClientNow();
